...
export default function Home() {
  ...
  const loadTrendingPage = useCallback(async (page: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTrendingPage(data.page || page);
  ...
  const handleSearch = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setPhase("done");
      setStatusText(data.results?.length > 0 ? `Found ${data.results.length} result${data.results.length !== 1 ? "s" : ""}` : "No results found. Try a different search term.");
    } catch (err: unknown) {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="border-b border-border-subtle bg-bg-secondary/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <a href="/docs" className="text-xs sm:text-sm text-text-muted hover:text-white transition-colors duration-200 border border-border-subtle rounded-lg px-3 py-1.5 hover:border-border-bright">API Docs</a>
        </div>
      </header>
            ...
            <form onSubmit={handleSearch} className="relative">
              <div className="search-glow rounded-2xl transition-all duration-300 bg-bg-card">
                <div className="flex items-center">
                  ...
                  <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search manga, manhwa, manhua, anime..." className="flex-1 bg-transparent py-4 sm:py-5 text-sm sm:text-base text-white placeholder-text-muted outline-none" autoFocus />
                  {query && (
                  ...
                  <button type="submit" disabled={!query.trim() || isSearching} className="mr-2 sm:mr-3 px-4 sm:px-6 py-2 sm:py-2.5 bg-white text-black font-medium text-sm rounded-xl hover:bg-gray-200 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200">Search</button>
                </div>
              </div>
            </form>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-6 sm:mt-8 pb-12 w-full">
              ...
              {[...Array(6)].map((_, i) => (
                <div key={i} className="glass-card rounded-xl p-4 sm:p-5 animate-pulse">
                  <div className="flex gap-4">
                    <div className="w-16 h-22 sm:w-20 sm:h-28 rounded-lg bg-bg-hover flex-shrink-0" />
          ...
          {phase === "done" && results.length === 0 && (
            <div className="text-center py-12 animate-fade-in-up">
              ...
              <button onClick={clearSearch} className="mt-4 text-xs text-text-secondary hover:text-white transition-colors underline">Browse trending instead</button>
            </div>
          )}
          ...
          {showTrendingSection && !loadingTrending && !loadingPage && trendingResults.length > 0 && (
            <div className="mt-8 animate-fade-in-up">
              <Pagination currentPage={trendingPage} hasMore={hasMorePages} onPageChange={goToPage} />
            </div>
          )}
        </div>
      </main>
      ...
      <footer className="border-t border-border-subtle py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            <span className="text-text-muted text-xs flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />All systems operational</span>
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-border-subtle bg-bg-card text-text-secondary text-xs sm:text-sm hover:bg-bg-hover hover:text-white hover:border-border-bright disabled:opacity-30 disabled:cursor-not-allowed transition-all"
        >
        ...
        {getVisiblePages().map((p, i) =>
          p === "..." ? (
            <span key={`dots-${i}`} className="px-1.5 sm:px-2 py-2 text-text-muted text-xs sm:text-sm">…</span>
          ) : (
            <button
              key={p}
              onClick={() => onPageChange(p)}
              className={`min-w-[36px] sm:min-w-[40px] h-9 sm:h-10 rounded-xl text-xs sm:text-sm font-medium transition-all ${
                p === currentPage
                  ? "bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.15)]"
                  : "border border-border-subtle bg-bg-card text-text-secondary hover:bg-bg-hover hover:text-white hover:border-border-bright"
              }`}
            >
              {p}
            </button>
    ...
    <div className="relative w-48 h-48 sm:w-60 sm:h-60 md:w-72 md:h-72">
      ...
      <svg className="absolute inset-0 w-full h-full animate-spin-slow" viewBox="0 0 300 300">
        ...
        <text className="fill-white/50" style={{ fontSize: "13px", letterSpacing: "5px", fontWeight: 600, fontFamily: "system-ui, sans-serif" }}>
          <textPath href="#circlePath" startOffset="0%">{circleText}</textPath>
        </text>
      </svg>
      <svg className="absolute inset-0 w-full h-full animate-spin-slow-reverse" viewBox="0 0 300 300">
        <defs><path id="innerCirclePath" d="M 150,150 m -75,0 a 75,75 0 1,1 150,0 a 75,75 0 1,1 -150,0" /></defs>
        <text className="fill-white/20" style={{ fontSize: "9px", letterSpacing: "3px", fontWeight: 400, fontFamily: "system-ui, sans-serif" }}>
          <textPath href="#innerCirclePath" startOffset="0%">MANGA · MANHWA · ANIME · DONGHUA · MANHUA · WEBTOON · </textPath>
        </text>
      </svg>
            ...
            <div key={step.key} className="flex items-stretch gap-3 sm:gap-4">
              <div className="flex flex-col items-center">
                <div className={`relative z-10 flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 transition-all duration-500 flex-shrink-0 ${done ? "border-green-500 bg-green-500" : active ? "border-green-400 bg-green-500/20 shadow-[0_0_12px_rgba(34,197,94,0.4)]" : "border-border-bright bg-bg-card"}`}>
                  {done ? <svg className="w-3.5 h-3.5 text-black" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                    : active ? <div className="w-2.5 h-2.5 rounded-full bg-green-400 animate-pulse" />
      ...
      <div className="w-full sm:max-w-2xl max-h-[90vh] bg-bg-secondary border border-border-subtle rounded-t-2xl sm:rounded-2xl overflow-hidden animate-fade-in-up flex flex-col">
        <div className="flex items-start justify-between p-5 sm:p-6 border-b border-border-subtle">
          <div className="flex gap-4 flex-1 min-w-0">
            {result.coverUrl ? (
              <div className="w-20 h-28 sm:w-24 sm:h-32 rounded-lg overflow-hidden flex-shrink-0 bg-bg-hover"><img src={result.coverUrl} alt="" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} /></div>
            ) : (
          ...
          <button onClick={onClose} className="ml-3 p-2 rounded-lg hover:bg-bg-hover transition-colors text-text-muted hover:text-white flex-shrink-0"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5">
                  ...
                  {result.chapters.map((ch, idx) => (
                    <a key={idx} href={ch.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between px-4 py-2.5 hover:bg-bg-hover transition-colors group">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                        ...
                        <svg className="w-3.5 h-3.5 text-text-muted group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                      </div>
                    </a>
                  ))}
                </div>
          ...
          <a href={result.url} target="_blank" rel="noopener noreferrer" className="block w-full text-center py-3 rounded-xl bg-white text-black font-medium text-sm hover:bg-gray-200 transition-colors">Visit Source →</a>
        </div>
      </div>
    </div>
  );
