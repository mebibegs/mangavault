...
const ALL_GENRES = [
  ...
  "Comedy","Demons","Drama","Fantasy","Gender Bender",
  "Harem","Heartwarming","Historical","Isekai","Josei",
  "Long Strip","Magic","Manga","Manhua","Manhwa",
  "Martial Arts","Mecha","Monsters","Mystery",
  "One Shot","Psychological","Reincarnation","Revenge",
  "Romance","School Life","Shoujo","Shounen",
  "Sci-Fi","Thriller","Slice of Life","Super Power",
  "Supernatural","Survival","Time Travel","Tragedy",
  "Transmigration","Vampires","Villainess","Webtoons",
  "Murim","Regression","System","Tower","Dungeon",
  "Overpowered","Genius MC","Crazy MC","Villain",
  "Violence","Necromancer","Game","Superhero",
  "Graphic Novel","Wuxia","Xianxia","Cultivation",
  "Military","Cyberpunk","Steampunk","Space Opera",
  "Crime","Detective","Battle Royale","Post-Apocalyptic",
  "Dystopian","Monster","Zombies","Ghosts","Aliens",
  "Time Loop","Hunter","Academy","Kingdom Building",
  "Politics","Family","Coming of Age","Parody",
  "Cooking","Gaming","Esports","Mythology","Folklore",
  "Occult","Alchemy","Virtual Reality","Psychic",
  "Transformation","Dragons","Farming","Slow Life",
...
function GenresPageContent({ initialGenre, initialResults }: GenresClientProps) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const selectedGenre = searchParams.get("q") || initialGenre || ALL_GENRES[0];
  // Seed with SSR data so the page is never blank on first paint
  const [results, setResults] = useState<MangaResult[]>(initialResults);
  const [loading, setLoading] = useState(false);
  const [selectedResult, setSelectedResult] = useState<MangaResult | null>(null);
  const [showChapters, setShowChapters] = useState(false);
  // Track which genre the SSR data belongs to so we don't re-fetch unnecessarily
  ...
  useEffect(() => {
    // Skip the fetch if we already have SSR data for this genre
    if (selectedGenre === ssrGenreRef.current && initialResults.length > 0) {
      ssrGenreRef.current = ""; // only skip once
      return;
    }
    search(selectedGenre);
  }, [selectedGenre, search, initialResults.length]);
...
export default function GenresClient({ initialGenre, initialResults }: GenresClientProps) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-bg-primary flex items-center justify-center"><p className="text-text-muted">Loading...</p></div>}>
      <GenresPageContent initialGenre={initialGenre} initialResults={initialResults} />
    </Suspense>
  );
}
