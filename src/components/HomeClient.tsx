...
// ═══════════════════════════════════════════════════════════════
// Custom image loader for /api/img (Issue 1)
// ═══════════════════════════════════════════════════════════════
function imgLoader({ src, width, quality }: { src: string; width: number; quality?: number }) {
  // src is the raw cover URL already proxied through /api/img
  if (src.startsWith("/api/img")) {
    // Already has ?url=... — append width
...
function GenreSection({ onCardClick }: { onCardClick: (r: MangaResult) => void }) {
  ...
  const fetchGenre = useCallback(async (genre: string) => {
    ...
    try { const res = await fetch(`/api/genres?q=${encodeURIComponent(genre)}`); if (res.ok) { const d = await res.json(); setGenreResults((d.results || []).slice(0, MAX_GENRE_CARDS)); } } catch { /* */ }
